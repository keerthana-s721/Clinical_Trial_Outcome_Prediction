...
@st.cache_data
def load_data():
    df = pd.read_csv('‪D:\10th sem project\db2\clinical_trial_cleaned.csv')
    return df
...
# Disease related to therapies, pharmacokinetics, and efficacy
disease_treatment = filtered_df[['disease', 'e_therapy', 'e_safety', 'e_efficacy', 'e_pharmacokinetic']].dropna()
fig_disease_treatment = px.sunburst(disease_treatment, path=['disease', 'e_therapy', 'e_efficacy'],
                                    title="Disease and Treatment Overview")
...
# Show a list of IMPs and related drug details
